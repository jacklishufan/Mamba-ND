...
from models.mamband import mamba3d_small
...
def main(args, ds_init):
    ...
    if args.local_rank == 0:
        wandb.init(project="video_mamba", sync_tensorboard=True)
    patch_size = (16,16) # model.patch_embed.patch_size
    ...
    if args.finetune:
        ...
        if 'deit' in args.model or 'videomamba' in args.model:
            ...
            # # we use 8 frames for pretraining
            orig_t_size = args.orig_t_size // 1
            new_t_size = args.num_frames //1
            orig_xy_size = int(((pos_embed_checkpoint.shape[-2])//(orig_t_size)) ** 0.5)
            orig_size = (orig_t_size,orig_xy_size,orig_xy_size)
            new_size = model.patch_resolution #int((num_patches // new_t_size) ** 0.5)
            if orig_size != new_size:
                
                tmp_pos_embed = pos_embed_checkpoint.view(1, orig_t_size, orig_xy_size,orig_xy_size, embedding_size)
                tmp_pos_embed = tmp_pos_embed.permute(0, 4,1,2,3) # H C L^2 
                tmp_pos_embed = torch.nn.functional.interpolate(tmp_pos_embed, size=new_size, mode='trilinear')
                tmp_pos_embed = tmp_pos_embed.permute(0,2,3,4,1).flatten(1,3)
