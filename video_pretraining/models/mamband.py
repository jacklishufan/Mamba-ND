                ...
                if (shape_1:=curr_state_dict[k].shape) != (shape_2:=state_dict[k].shape):
                    if 'patch_embed' in k:
                        ...
                        try:
                            ...
                            state_dict[k] = curr_state_dict[k]
                    else:
...
@register_model
def mamba3d_small(pretrained=False, **kwargs):
    model = Mamba3DModel(
        arch='small',
        d_state=16,
        # drop_path_rate=0.1,
       ...
       # num_frames=16,
        ...
        patch_size=16,
        patch_size_temporal=1,
        pretrained='/home/jacklishufan/mamband/video_classification/300_v1.pth',
        type='VisionMamba',
        use_mlp=False,
        use_nd=False,
        use_v2=False,
...
if __name__ == '__main__':
    ...
    model = mamba3d_small().cuda()
    y = model(torch.rand(1, 3, img_size, img_size).cuda())
    print(y.shape)
